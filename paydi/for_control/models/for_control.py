...
class ForControl(models.Model):
    ...
    data_tree_file = fields.One2many(
        comodel_name="file.data", string="data file", inverse_name="file_name_id"
    )
    ...
    def import_file(self):
        if self.file_data:
            ...
            if response.json()["error_code"] == "":
                ...
                message_id = self.env["popup.notification"].create(
                    {
                        "message": (
                            "Upload file không thành công, vui lòng kiểm tra lại file !"
                        )
                    }
                )
                return {
            ...
            wb = openpyxl.load_workbook(
                filename=BytesIO(base64.b64decode(self.file_data)), read_only=True
            )
            ws = wb.active
            sheet = wb["Sheet1"]
    ...
    def action_from_view(self):
        ...
        if self.status_save_file == 1:
            body_obj = {"file_name": self.file_name}
            back_end_url = os.getenv("URL_PREFIX")

            result = requests.post(
                f"{back_end_url}/v1/odoo-api/fund_transfer/transfer", json=body_obj
            )
            ...
            if result.json()["data"]["result"] == "success":
                ...
                result.json()["data"]["msg"].find("ConnectTimeoutError") != -1
                or result.json()["data"]["msg"].find("NewConnectionError") != -1
            ):
                message_id = self.env["popup.notification"].create(
            ...
            message_id = self.env["popup.notification"].create(
                {
                    "message": (
                        "Chuyển tiền không thành công, vui lòng kiểm tra lại file !"
                    )
                }
            )
            return {
    ...
    def get_all_trans_installment(self):
        ...
        transaction_late = self.env["transaction.installment"].search_read([], limit=1)
        _created_time = 0
                    ...
                    self.env["transaction.installment"].create(
                        {
                            ...
                            "contact_id": trans.get("odoo_contact_id"),
                        }
                    )
